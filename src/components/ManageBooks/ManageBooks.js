...
const ManageBooks = () => {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch(`https://agile-mountain-29052.herokuapp.com/tickets`)
      .then((res) => res.json())
  ...
  const updateStatus = (id) => {
    ...
    fetch(`https://agile-mountain-29052.herokuapp.com/update/${id}`, {
      method: "PUT",
      headers: {
  ...
  const deleteTicket = (id) => {
    ...
    if (confirmation) {
      const url = `https://agile-mountain-29052.herokuapp.com/myOrders/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
